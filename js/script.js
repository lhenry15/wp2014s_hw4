...
window.fbAsyncInit = function () {
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
			...
			ctx.drawImage(profileIMG,canMouseX-128/2,canMouseY-120/2); //根據你的滑鼠游標移動，你可以自行更換想要移動的圖層，數值會因XY軸向有所不同
			ctx.drawImage(img3,canMouseX-offsetX/2,canMouseY-offsetY);
			ctx.drawImage(img2,0,0); //劃入img2
			var inputedText = $('#inputed').val();//抓取頁面inputed ID的內容
			ctx.fillStyle = "black"; //字體顏色
			ctx.font='20px "微軟正黑體"'; //字體大小和字形
			ctx.fillText(inputedText, offsetY,offsetX/2); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
